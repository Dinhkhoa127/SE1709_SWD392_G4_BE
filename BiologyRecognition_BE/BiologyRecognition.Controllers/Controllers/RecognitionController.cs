...
namespace BiologyRecognition.Controller.Controllers
{
    ...
    public class RecognitionController : ControllerBase
    {
        ...
        public RecognitionController(IUserAccountService userAccountService, IMapper mapper, ISubjectService subjectService, IChapterService chapterService, IRecognitionService recognitionService, HttpClient httpClient, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> Recognize([FromBody] ImageDTO imageDTO)
        {
            try
            {
                ...
                if (aiResult.Confidence < 0.7)
                    return Ok(new { success = false, message = "Không nhận dạng được." });
                ...
                return Ok(new { success = true, artifact = aiResult });
            }
            catch (Exception ex)
            {
